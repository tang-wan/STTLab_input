...
class TB_JPinput():
    ...
    def __init__(self, 
        ...
        self.pbsPath    = f"{RLLgroup_sub}/{RLLnum_sub}/{RLLnum_angle_sub}/{RLLnum_angle}"
        self.workPath   = f"{RLLnum}/"
    ...
    def MK_JPpbsALL(self, init=False,
                    name='current', machine='i91', ppn=18):
        pbsPath = self.pbsPath
        pbsname = f"RLL{len(self.NL_set)}_junpyRunALL.pbs"
        self.pbsname = pbsname
        init_lines = [
                    '#!/bin/bash\n',
                    '#=============================================#\n',
                    '# Job Name\n',
        ...
        if init:
            with open(f'{self.RLLnum}/{pbsname}', 'w') as W_File:
                W_File.writelines(init_lines)
                W_File.writelines(run_lines)
        else:
            with open(f'{self.RLLnum}/{pbsname}', 'a') as A_File:
                A_File.writelines(run_lines)

    def RunCodeALL(self, Path):
